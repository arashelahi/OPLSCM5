...
# Try gmx first
$prefix = `gmx -help 2>&1 | grep "Data prefix" | awk -F':' '{print \$2}' | xargs`;
# print "Fallback\n" if $prefix !~ /\//;  # prints "Fallback"
# print "GROMACS dir: $prefix\n";

# If gmx failed (empty prefix), try gmx_mpi
if ($prefix !~ /\//) {
    $prefix = `gmx_mpi -help 2>&1 | grep "Data prefix" | awk -F':' '{print \$2}' | xargs`;
}

chomp($prefix);  # Remove trailing newline
$gromacs_dir = "$prefix/share/gromacs/top";
print "GROMACS dir: $gromacs_dir\n";

# $gromacs_dir='/sw/spack/deltas11-2023-03/apps/linux-rhel8-zen3/gcc-11.4.0/gromacs-2022.5-clsbrxa/share/gromacs/top/';
...
for ($i = 1; $i <= $linha; $i++) {
 if ($btype[$i] eq "C" or $btype[$i] eq "C_2" or $btype[$i] eq "C_3") {
  ...
  $j = 0;
  foreach $atom (@array) {
   if ($btype[$atom] eq "O" or $btype[$atom] eq "O_2" or $btype[$atom] eq "O_3") {
    $j = $atom; 
   }
   else {
    $k[$a]=$atom;
  ...
  if($forcefield eq "opls")
  {
   $improper[$I]="$j $i $k[0] $k[1]  4    improper_O_C_X_Y";
  }
  if($forcefield eq "amber")
  {
 ...
 if ($btype[$i] eq "CM" or $btype[$i] eq "C=") {
  ...
  if($forcefield eq "opls")
  {
   $improper[$I]="$array[0] $array[1] $i $array[2] 4   improper_Z_CM_X_Y";
  }
  if($forcefield eq "amber")
  {
